...
class UnitreeEnv(Environment):
    ...
    def update(self):
        ...
        if self.robot == "g1":
            quat = np.asarray(self.low_state.imu_state.quaternion, dtype=np.float32)[[1, 2, 3, 0]]
            ...
            torso_quat = np.asarray(self.low_state.imu_state.quaternion, dtype=np.float32)[[1, 2, 3, 0]]
